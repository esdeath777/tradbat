...
if (Get-Command winget -ErrorAction SilentlyContinue) {
    ...
    Copy-Item `
        -LiteralPath $downloadedExe.FullName `
        -Destination $fastfetchLocalPath `
        -Force

    Unblock-File `
        -LiteralPath $fastfetchLocalPath `
        -ErrorAction SilentlyContinue
...
if (-not ($pathItems | Where-Object {
    $_.TrimEnd('\') -ieq $binDir.TrimEnd('\')
})) {
    [Environment]::SetEnvironmentVariable(
        'Path',
        (($pathItems + $binDir) -join ';'),
        'User'
    )
}
